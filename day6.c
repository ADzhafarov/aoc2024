...
int check_for_loop_option(Coordinates current, Coordinates direction, char *rows[], int height, int width, Coordinates start);
Coordinates step(Coordinates current_coord, Coordinates *direction, int height, int width, char *rows[], int *full_turn);


int main(void) {
    char *rows[HEGITH] = {NULL};
    ...
    while (i < HEGITH && (line_len = getline(rows + i++, &maxrowlen, stdin)) > 0)
    ...
    while (1) {
        ...
        loop_possible = check_for_loop_option(current_coord, direction, rows, real_height, real_width, start);
